...
module.exports =
    ...
    class PubSubChat extends floyd.gui.ViewContext
        ...
        _receive: (err, msg)->
            ...
            if data = msg.data
                ...
                if text = data.text
                    ...
                    if text.substr(0, 11) is '_ENCRYPTED_'
                        text = @_createDecryptable msg, text.substr(11)

                    @_write $('<span><b style="'+style+'">'+nick+': </b></span>').append text
        ...
        ##
        ##
        _createDecryptable: (msg, text)->
            span = $ '<span>Verschlüsselte Nachricht von: '+msg.data.nick+'. <a href="#">entschlüsseln</a></span>'

            span.find('a').click ()=>
                pass = prompt 'Passworteingabe erforderlich.'
                if pass
                    span.text floyd.tools.crypto.decrypt text, pass
        ...
        ##
        _write: (line)->
            @_display.append $('<li>').append line
